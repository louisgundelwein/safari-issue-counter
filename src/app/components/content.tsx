...
export default function Content({ incidents }: { incidents: Incident[] }) {
	...
	return (
		<div className="flex flex-col justify-center items-center py-8 gap-8">
			<h1 className="font-extrabold text-4xl m-8 text-center text-primary">
				{'Safari is a very good and sane browser, everyone should use it ._.'}
			</h1>
			<DayCounter incidents={incidents} />
			<div className="flex flex-col justify-start items-center">
				<button
					className="bg-primary-light text-black text-2xl px-4 py-2 rounded-full shadow cursor-pointer  hover:scale-110 transform transition-transform duration-200"
					onClick={() => setShowAddModal(true)}
				>
					+
				</button>
				{incidents.length > 0 && <div className="w-[1px] h-12 bg-primary-light"></div>}
					...
					return (
						...
						>
							...
							{index < incidents.length - 1 && (
								<div className="w-[1px] h-12 bg-primary-light"></div>
							)}
						</div>
					);
				})}
